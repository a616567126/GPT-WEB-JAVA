...
## 💻  INSTALL AND START  
       安装步骤
        ...
        7.若使用支付，检查pay_conf，可使用微信支付，易支付，自行选择对应支付方式
        8.修改yml中的数据库配置与redis配置
        9.在根目录下创建临时上传路径/www/temp/data 或自己根据自己实际的路径来配置，注意修改yml中第17行
        10.增加上传图片目录/www/uploads/ 或自己根据自己实际路径来配置，注意修改sys_config表中的图片上传路径
        11.打包java代码复制到/usr/local/siana/下，后续都已这个为准，可根据自己的项目实际位置自行更改
        12.执行nohup java -jar Intelligent-Bot-0.0.1-SNAPSHOT.jar 启动后台
        13.查看日志是否有异常日志，若没有则启动成功
        14.管理员账号admin密码123456，根据自己需求合理增加或修改表内数据，初始化sql只为正常启动代码
        ----------------以下说明为客户端、管理端、移动端部署说明---------------------
        15.在服务器上配置启动好服务之后，在客户端与管理端修改api.js里的接口地址,
           PC端与移动端App.js中修改对应的页面地址，siana_bot（填写移动端地址），
           siana_mobile（填写PC端地址），配置好后自动适配设备，
           PC端管理端使用Node14,Npm6其他版本可能出现各种异常情况
        16.使用npm run dev 测试是否接口请求正常。，
        17.使用npm run build 打包代码，并部署到服务器
        18.使用Hbuilder打开移动端代码，打包方式（网站-PC Web或手机H5(仅适用于 uni-app)）
